...
uint16_t  lastkey=0;
BOOL      f_lowpower;  									//低功耗调用标志,休眠时置1调用 TouchKeyScan() 可降低功耗
BOOL      f_gotosleep;
...
//***********************************************************************
void WakeupInitial(void)  					//醒之后IO设置
{
	...
	{       
           ...
           if(TouchTimeCount>=(1000/SCANNINGTIME))   			
		   {
			...
			else
			{
				 lastkey=0;
				f_gotosleep = 0;
				if(f_gotosleep == 0)  						//f_gotosleep等于0，进入休眠
				{  
					if(TouchTimeCount1>=1000)	//1s
					{
						...
						while(1)
                        {
							...
							f_lowpower=1;    							//以低功耗的方式调用TouchKeyScan();
