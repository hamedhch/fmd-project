...
int ReadAPin  , R=0 , G=0, B=0 ;

unsigned char	 ir_status , ir_timer , Counter ,Set=1, mode ,re , repeat , Counter_On_set , Counter_On,s,m,h,CountINT ,RGB , timer , one , modetime;
...
void main()
{
	...
	while(1)
	{
		...
		if(mode==3){
			if((CountINT==75||CountINT==0)&&re==0)re=1,RGB++;
			
			if(RGB==0) R=180 , G=180, B=0 ;
			if(RGB==1) R=180 , G=0  , B=180 ;
			if(RGB==2) R=0   , G=180, B=180 ;
			if(RGB>=3) RGB=0;
		}
		...
		if(mode==4&&Set==1){
			...
			if(s%2==0){
				if(CountINT%15==0)LED_1=1,LED_2=1,LED_3=1;
                if(CountINT%15==8)R=180 , G=180 , B=180 ,LED_1=0,LED_2=0,LED_3=0;
            }else {
				if(RGB==1) LED_1=1,LED_2=0,LED_3=0 ;
				if(RGB==2) LED_1=0,LED_2=1,LED_3=0;
				if(RGB==3) LED_1=0,LED_2=0,LED_3=1;
				if(RGB>=3) RGB=0;
            }
		...
		if(mode>=4&&Set==0)LED_1=0,LED_2=0,LED_3=0;
		...
		if(ReceiveFinish ||  repeat)
        {
            ...
            if((rdata1 == IRDataTimer[1])&&(rdata2 == IRDataTimer[3]))
            {
                ...
                {
					mode=0;
                    if(one==0)one=1,RGB=0 , R=180 , G= 180 , B=0;
                    
                    if(RGB==0) R=180 , G=180, B=0 ;
                    if(RGB==1) R=180 , G=0,   B=0 ;
					if(RGB==2) R=180 , G=0  , B=180 ;
                    if(RGB==3) R=0   , G=0, B=180 ;
					if(RGB==4) R=0   , G=180, B=180 ;
                    if(RGB==5) R=0   , G=180, B=0 ;
                    if(RGB==6) R=0   , G=0, B=0 ;
                ...
                {
					if(Counter_On_set>0){
						Counter_On_set=Counter_On_set-6;
